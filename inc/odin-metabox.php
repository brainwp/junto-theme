...
class Odin_Metabox {
	 ...
	 * @return void
	 */
	public function __construct( $id, $title, $post_type = 'post', $context = 'normal', $priority = 'high', $pageID = false ) {
		$this->id        = $id;
		$this->title     = $title;
	...
	public function scripts() {
		...
		if ( in_array( $screen->id, $this->get_post_type() ) ) {
			...
			// Metabox.
            wp_enqueue_script( 'odin-admin', get_template_directory_uri() . '/js/odin-admin.js', array( 'jquery' ), null, true );
            wp_enqueue_style( 'odin-admin', get_template_directory_uri() . '/css/odin-admin.css', array(), null, 'all' );
	...
	public function metabox( $post ) {
        ...
        $remove = '';
        if($post_id != $this->pageID || $this->pageID == false){
            $remove = 'brasa-remove-box';
        }
        ...
        echo apply_filters( 'odin_metabox_container_before_' . $this->id, '<table data-box-id="'.$this->id.'" class="form-table odin-form-table '.$remove.'">' );
